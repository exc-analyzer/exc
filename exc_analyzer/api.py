...
from exc_analyzer.config import load_key, secure_store_available
...
def notify_new_version():
    def _check():
        ...
        try:
            ...
            from exc_analyzer.logging_utils import log
            log(f"Version comparison failed: {e}")
            ...
            if resp_headers.get('X-RateLimit-Remaining') == '0':
                ...
                if reset:
                    try:
                        ...
                        from exc_analyzer.logging_utils import log
                        log(f"Rate-limit reset parsing failed: {e}")
...
def get_auth_header():
    if not secure_store_available():
        from exc_analyzer.i18n import t
        print("")
        Print.error(t("commands.shared.keyring_required"))
        Print.info(t("commands.shared.keyring_required_hint"))
        Print.info(t("commands.shared.keyring_required_faq"))
        print("")
        sys.exit(1)
    key = load_key()
    if not key:
...
def poll_for_token(device_code: str, interval: int):
    ...
    while time.time() < expires_at:
        try:
            ...
            from exc_analyzer.logging_utils import log
            log(f"Token polling request failed: {e}")
        time.sleep(interval)
