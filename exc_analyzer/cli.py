...
from exc_analyzer.print_utils import Print, COLOR_ENABLED, colorize, set_verbose, close_output_file, print_error
    ...
    if hasattr(args, 'func'):
        try:
            ...
            try:
                ...
                log(f"Output close failed on keyboard interrupt: {e}")
            sys.exit(130)  
        except Exception as e:
            try:
            ...
            try:
                ...
                from exc_analyzer.logging_utils import log
                log(f"Output close failed on command exception: {ex}")
            from exc_analyzer.logging_utils import log
