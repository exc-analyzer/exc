...
from ..api import api_get, safe_get, get_auth_header, DEFAULT_TIMEOUT
from ..i18n import t
from .dork_presets import PRESETS
from ..helpers import _truncate, truncate_visual, get_visual_width
...
def _verify_content(item, headers):
    ...
    try:
        ...
        if 'github.com' in html_url and '/blob/' in html_url:
            ...
            text, _, status = safe_get(raw_url, timeout=DEFAULT_TIMEOUT, cacheable=True)
            if status == 200:
                content = text
                return _analyze_content(content)
    except Exception as e:
        from ..logging_utils import log
        log(f"Dork verify content failed: {e}")
...
def shorten_display_url(full_url, max_len):
    ...
    try:
        ...
        from ..logging_utils import log
        log(f"Display URL shorten failed: {e}")
        return _truncate(full_url, max_len)
